...
mongoose.connect(process.env.DB_URL, { useNewUrlParser: true, useUnifiedTopology: true }).then(() => console.log('connected to database successfully')).catch(err => console.log(`error connecting to database: ${err}`));
...
io.on('connection', (socket) => {
    ...
    socket.on('conversation', (map) => {
        ...
        const conversation = map.conversation; /// it's going to be a room
        if (!conversation) return console.log('conversation or room is not defined');
        ...
        if (io.sockets.adapter.rooms.has(conversation)) {
            if (io.sockets.adapter.rooms.get(conversation).has(socket.id)) {
                ...
                return console.log(`room ${conversation} already created or socket ${socket.id} already joined, so just emit the conversation event.`);
            } else {
                socket.join(conversation);
                return console.log(`room ${conversation} just created and socket ${socket.id} joined.`);
            }
        } else {
            socket.join(conversation);
            return console.log(`socket ${socket.id} joined room ${conversation} for the first time.`);
        }
    },
    );
},
...
/// start the server
server.listen(process.env.PORT, () => console.log(`server is running on port ${process.env.PORT}`)).on('error', err => console.log(`error in starting server: ${err}`));
